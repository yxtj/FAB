...
void RNNProxy::init(const std::string& param) {
...
int RNNProxy::lengthParameter() const
{
	return weightOffsetLayer[nLayer];
}

void RNNProxy::createLayerAct(const size_t i, const int n, const std::string& type) {
	nNodeLayer[i] = n;
	if (type == "sigmoid")
...
void RNNProxy::createLayerConv(const size_t i, const int n, const std::vector<int>& shape) {
	nNodeLayer[i] = n;
...
void RNNProxy::createLayerPool(const size_t i, const int n, const std::string& type, const std::vector<int>& shape) {
	nNodeLayer[i] = n;
	if (type == "max")
...
void RNNProxy::createLayerFC(const size_t i, const int n) {
	nNodeLayer[i] = n;
...
std::vector<int> RNNProxy::getShape(const string& str) {
	return getIntList(str, "*");
}

int RNNProxy::getSize(const std::vector<int>& ShapeNode) {
	if (ShapeNode.empty())
		return 0;
	int r = 1;
...
void RNNProxy::setLayerParameter(const size_t i) {
...
void RNNProxy::generateNode(const size_t i)
{
